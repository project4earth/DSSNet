...
class ModelTester:
    ...
    def test(self, test_loader):
        ...
        try:
            ...
            with torch.no_grad():
                test_bar = tqdm(enumerate(test_loader), total=len(test_loader), desc='Inference', leave=True)
                for _, (inputs, labels) in test_bar:
                    inputs, labels = inputs.to(self.device, dtype=torch.float16), labels.to(self.device, dtype=torch.long)
            ...
            # Hitung GFLOPS
            gflops = self._calculate_gflops(inputs)
