...
def l2_norm(x, eps=1e-6):
    norm = torch.norm(x, p=2, dim=-2, keepdim=True).clamp(min=eps)
    return x / norm
...
class SharedSpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (x + self.gamma * weight_value).contiguous()
...
class DCSwin(nn.Module):
    def __init__(self,
        ...
        self.backbone = SwinTransformer(in_channels, embed_dim=embed_dim, depths=depths, num_heads=num_heads, frozen_stages=frozen_stages)
