...
class ModelTrainer:
    ...
    def _train_one_epoch(self, epoch):
        ...
        for step, (sentinel, labels) in training_bar: 
            sentinel = sentinel.to(self.device, dtype=torch.float32)
            labels = labels.to(self.device, dtype=torch.float32)

            skip_batch = False
            with torch.cuda.amp.autocast():
                outputs = self.model(sentinel)

                # Cek apakah ada auxiliary loss
                if isinstance(outputs, tuple):
                    main_out, aux_out = outputs
                    loss_main = self.criterion(main_out, labels)
                    loss_aux = self.criterion(aux_out, labels)
                    loss = loss_main + 0.3 * loss_aux  # 0.4 bisa kamu ganti sesuai kebutuhan
                    used_output = main_out  # untuk metric
                else:
            ...
            f1_sum_train += self.metric_calculator.f1_score(used_output, labels).item() * batch_size
            acc_sum_train += self.metric_calculator.pixel_accuracy(used_output, labels).item() * batch_size
    ...
    def _validate(self, epoch):
        ...
        for step, (sentinel, labels) in validation_bar:
            sentinel = sentinel.to(self.device, dtype=torch.float32)  
            labels = labels.to(self.device, dtype=torch.float32)
